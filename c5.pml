...
proctype Process2() {
    do
    :: EXEC_WHEN_CURRENT(2, skip);
    :: EXEC_WHEN_CURRENT(2, EP = 1)
    od
}

/***** Helper macros for checking if task is in queues *****/
#define TASK_IN_READY_QUEUE(taskId) \
    (readyQueue[0].tasks[0] == taskId || readyQueue[0].tasks[1] == taskId || \
     readyQueue[0].tasks[2] == taskId || readyQueue[0].tasks[3] == taskId || \
     readyQueue[0].tasks[4] == taskId || \
     readyQueue[1].tasks[0] == taskId || readyQueue[1].tasks[1] == taskId || \
     readyQueue[1].tasks[2] == taskId || readyQueue[1].tasks[3] == taskId || \
     readyQueue[1].tasks[4] == taskId || \
     readyQueue[2].tasks[0] == taskId || readyQueue[2].tasks[1] == taskId || \
     readyQueue[2].tasks[2] == taskId || readyQueue[2].tasks[3] == taskId || \
     readyQueue[2].tasks[4] == taskId || \
     readyQueue[3].tasks[0] == taskId || readyQueue[3].tasks[1] == taskId || \
     readyQueue[3].tasks[2] == taskId || readyQueue[3].tasks[3] == taskId || \
     readyQueue[3].tasks[4] == taskId)

/***** LTL Properties: Running tasks must not be in ready queues *****/
/* 
 * Property: If a task is RUNNING, it must not be in any ready queue
 * Formula: [] (RUNNING -> !IN_READY_QUEUE)
 * In our model: [] (tcb[i].state == RUNNING -> !TASK_IN_READY_QUEUE(i))
 */

/* Task 1: If running, not in ready queue */
ltl running_not_in_queues_task1 { 
    [] ((tcb[1].state == RUNNING) -> !TASK_IN_READY_QUEUE(1))
}

/* Task 2: If running, not in ready queue */
ltl running_not_in_queues_task2 { 
    [] ((tcb[2].state == RUNNING) -> !TASK_IN_READY_QUEUE(2))
}

/* Combined property: both tasks must satisfy the property */
ltl running_not_in_queues_all {
    [] (((tcb[1].state == RUNNING) -> !TASK_IN_READY_QUEUE(1)) &&
        ((tcb[2].state == RUNNING) -> !TASK_IN_READY_QUEUE(2)))
}
