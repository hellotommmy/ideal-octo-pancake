...
### FreeRTOS
1. **创建任务**：调用 `xTaskCreate` 或 `xTaskCreateStatic` 时会分配/初始化 TCB，并通过 `prvAddNewTaskToReadyList` 将其加入就绪列表。如果这是系统的第一个任务，函数会把该任务赋给 `runTask`，同时调用 `prvInitialiseTaskLists` 初始化全部链表【266440765761988†L754-L779】。
2. **启动调度器**：应用调用 `vTaskStartScheduler()` 启动调度器。该函数会创建空闲任务（Idle Task），然后在进入调度前调用 `portDISABLE_INTERRUPTS()` 关闭中断，防止在启动过程中发生时钟中断【266440765761988†L1054-L1090】。随后调用 `ArchStartSchedule()` 进入任务调度。空闲任务和其他任务的栈中已保存启用中断的状态字，所以当首个任务运行时中断自动恢复【266440765761988†L1054-L1090】。

## 三、任务调度和中断控制

### LiteOS‑M
* **任务调度锁**：LiteOS‑M提供 `LOS_TaskLock`/`LOS_TaskUnlock`。锁定调度器会禁止任务切换，但不会关闭中断。`LOS_TaskLock` 实际上增加调度锁计数器，当计数器大于0时，当前任务会延迟调度；完成关键操作后调用 `LOS_TaskUnlock` 减少计数器，如果计数器降为0且有优先级更高的任务准备好，则立即调度【507159429201817†screenshot】【416659554099318†screenshot】。
* **中断锁**：使用 `LOS_IntLock`/`LOS_IntRestore` 来屏蔽/恢复中断，这通过设置 CPU 的中断使能位实现，用于保护临界区。它会返回之前的中断标志，配对调用才能正确恢复中断。
* **优先级调度**：内核支持 32 级优先级，调度器根据最高优先级就绪任务运行，优先级相同的任务采用时间片轮转【685104102085185†screenshot】。

### FreeRTOS
* **临界区保护**：使用 `taskENTER_CRITICAL`/`taskEXIT_CRITICAL` 宏关闭/恢复中断，保证对共享数据结构（如就绪链表、调度器变量）的访问具有原子性。底层实现调用 `portDISABLE_INTERRUPTS()` 和 `portENABLE_INTERRUPTS()`，具体取决于移植层。
* **调度挂起/恢复**：FreeRTOS提供 `vTaskSuspendAll()` 和 `xTaskResumeAll()` 用于挂起调度器。与 LiteOS‑M 的任务锁类似，挂起调度时内核不会立即切换任务，但中断仍然运行。挂起期间新就绪任务被插入到 `xPendingReadyList`，在恢复时再统一移动到对应就绪链表。这与 LiteOS‑M 的调度锁类似，但采用链表移动实现。
* **任务优先级**：FreeRTOS优先级数量由 `configMAX_PRIORITIES` 配置（通常 5~32）。调度器总是选择优先级最高的就绪任务运行；若多个任务处于相同优先级，则以时间片轮转方式运行。与 LiteOS‑M 相同，空闲任务优先级最低且始终存在。

## 四、实现细节比较

| 比较项 | LiteOS‑M | FreeRTOS | 说明 |
|---|---|---|---|
| **TCB存储方式** | `g_taskCBArray` / `g_pstTaskCBArray`：系统初始化时分配固定数量的 `LosTaskCB` 数组；通过索引获取具体 TCB【195179307722980†screenshot】。空闲 TCB 挂载在 `g_losFreeTask` 列表 | 动态分配 `TCB_t`；`runTask` 指向当前任务；就绪列表 `pxReadyTasksLists[]` 为每个优先级维护链表【266440765761988†L480-L513】 | LiteOS‑M使用静态数组，利于对内存使用的可控；FreeRTOS支持静态或动态分配，灵活性更高。 |
| **就绪队列** | `g_priQueueList[OS_PRIORITY_QUEUE_NUM]`：32个优先级对应32个双向链表【195179307722980†screenshot】 | `pxReadyTasksLists[configMAX_PRIORITIES]`：链表数组，数量由配置宏决定【266440765761988†L480-L513】 | 两者都根据优先级选择最高优先级任务。
| **创建首个任务** | 调用 `LOS_KernelInit` 初始化 TCB 数组、空闲链表、就绪队列并创建空闲任务【195179307722980†screenshot】；应用通常创建一个主任务（如 `OsMainTask`）并在 `LOS_Start` 启动调度；任务栈通过 `OsTaskStackInit` 预填 PC、LR、R0 等【164724710294744†screenshot】 | `xTaskCreate` 创建任务，通过 `prvAddNewTaskToReadyList` 将第一个任务加入就绪链表并将 `runTask` 指向它；`vTaskStartScheduler` 创建空闲任务后关闭中断并调用 `ArchStartSchedule` 进入调度【266440765761988†L754-L779】【266440765761988†L1054-L1090】 | LiteOS‑M必须先初始化内核并创建首个任务，再调用 `LOS_Start`；FreeRTOS把链表初始化和 `runTask` 指针设置放在创建首个任务时完成。
| **调度锁与中断控制** | `LOS_TaskLock`/`LOS_TaskUnlock` 锁定调度器但不屏蔽中断【507159429201817†screenshot】；`LOS_IntLock`/`LOS_IntRestore` 用于屏蔽/恢复中断 | `vTaskSuspendAll`/`xTaskResumeAll` 挂起调度，不禁用中断；`taskENTER_CRITICAL`/`taskEXIT_CRITICAL` 关闭/恢复中断 | 都区分了调度锁和中断锁，但实现机制不同：LiteOS‑M通过计数器控制调度锁；FreeRTOS使用挂起调度和链表移动实现。
| **任务状态及调度策略** | 定义任务状态：Ready、Running、Blocked、Dead【685104102085185†screenshot】；32级优先级，优先级相同用时间片调度【685104102085185†screenshot】 | 任务状态为就绪、运行、阻塞、悬挂、删除等待等；优先级数量由配置决定，同优先级时间片轮转 | 两者都支持抢占式调度和时间片；LiteOS‑M固定优先级数量，FreeRTOS可配置。

## 五、结论与建议

LiteOS‑M和FreeRTOS都是面向资源受限MCU的实时内核，两者在基本调度机制上类似：均采用基于优先级的抢占式调度，任务处于就绪队列，通过高优先级抢占低优先级任务。然而实现方式存在差异：

* **内存模型**：LiteOS‑M在内核启动时静态分配TCB数组和就绪队列，内存用量固定；FreeRTOS允许动态创建/删除任务，内核结构更加灵活但管理更复杂。
* **调度启动流程**：LiteOS‑M在 `LOS_KernelInit` 初始化所有内核对象并创建空闲任务，`LOS_Start` 后再执行主任务；FreeRTOS在创建第一个任务时初始化链表，调度器启动时创建空闲任务并短暂关闭中断。 
* **调度锁与中断控制**：两者都区分调度锁和中断锁，但LiteOS‑M提供单独的 `LOS_TaskLock`/`LOS_IntLock`；FreeRTOS使用挂起调度和临界区宏实现。理解这些差异有助于在移植或开发时正确使用调度器相关API，避免在临界区使用错误的锁而导致系统失去实时性或出现死锁。 -->


<!-- **参考资料：** LiteOS-M 任务调度实现细节。



* **任务初始化（就绪队列）:** 将 FreeRTOS 的 `prvInitialiseTaskLists()` 替换为 LiteOS-M 的 **`OsSchedInit()`**。即用 `LOS_ListInit(&g_priQueueList[p])` 初始化各优先级双向链表（就绪队列），并清零任务队列位图 `g_sched->queueBitmap`（对应 FreeRTOS 的 `uxTopReadyPriority = 0`）。
* **优先级位图:** FreeRTOS 的 `uxTopReadyPriority`（记录最高就绪优先级）替换为 LiteOS-M 的 **`g_sched->queueBitmap`**（多级位图，标记哪些优先级队列非空）。当优先级队列从空变为非空时，LiteOS-M 用 CLZ+位运算设置位图；从非空变为空时清除位图（对应 FR 的 `taskRESET_READY_PRIORITY` 逻辑）。
* **就绪队列插入/删除:** FreeRTOS 的 `vListInsertEnd(pxReadyTasksLists[p],…)` （在优先级 p 队列尾插入任务）对应 LiteOS-M 的 **`LOS_ListTailInsert(&queueList->priQueueList[p], item)`**；移除任务则对应 **`LOS_ListDelete(&queueList->priQueueList[p], item)`** 操作（LiteOS-M 具体函数名如 `LOS_ListDelete`）。检查队列是否空用 **`LOS_ListEmpty(&queueList->priQueueList[p])`**。
* **任务创建/就绪:** FreeRTOS 的 `xTaskCreate`（这里模型为 `xTaskCreate_fixed`）替换为 LiteOS-M 的 **`LOS_TaskCreate()`** 或对应底层 `OsTaskInit` 等接口（创建任务 TCB、初始化堆栈、设置优先级，并插入对应就绪队列）。例如，LiteOS-M 调用后自动将新任务放入就绪队列，必要时更新位图。
* **开始调度:** FreeRTOS 的 `vTaskStartScheduler()` 不直接对应 LiteOS-M 接口；LiteOS-M 在初始化后即可自动开始调度（如 `OsSchedInit()` 调用后通过系统时钟中断和 `LOS_Schedule()` 触发任务切换）。
* **调度触发:** FreeRTOS 的 `taskYIELD()`（或 `taskYIELD_IF_USING_PREEMPTION`）对应 LiteOS-M 的 **`LOS_Schedule()`**（也就是 `LOS_IntRestore` 后触发 `OsSchedResched()`）。Tick 中断或任务事件触发时，LiteOS-M 调用 `LOS_Schedule()` 来安排新的任务。
* **任务切换:** FreeRTOS 的 `taskSELECT_HIGHEST_PRIORITY_TASK` 则对应 LiteOS-M 内部的 **`OsGetTopTask()`**（在就绪队列位图中查找最高优先级并出队）加上 **`OsSchedTaskSwitch()`**（切出当前任务，切入新任务）。例如，LiteOS-M 用 CLZ 找到最高优先级后，用 `LOS_DL_LIST_FOR_EACH_ENTRY` 遍历该优先级队列获取任务，然后调用 `OsSchedDeTaskQueue()` 将其出队并运行。
* **任务挂起与恢复:** FreeRTOS 的 `vTaskSuspend`/`vTaskResume` 对应 LiteOS-M 的 **`LOS_TaskSuspend()`**/**`LOS_TaskResume()`**（或者 CMSIS-RTOS `osThreadSuspend`/`osThreadResume`）。挂起时从就绪队列中移除，恢复时插回就绪队列并可能触发调度。
* **任务延时:** FreeRTOS 的 `vTaskDelay` 对应 LiteOS-M 的 **`LOS_TaskDelay()`**（或 `osDelay`）。LiteOS-M 将当前任务从就绪队列移动到定时器链表或延时队列，并在延时到期时再插入就绪队列。
* **优先级变更:** FreeRTOS 的 `vTaskPrioritySet` 对应 LiteOS-M 的 **`LOS_TaskPriSet()`**。改变优先级时，同样需从原优先级队列移除并插入到新优先级队列中，更新 `queueBitmap`。
* **临界区（调度锁）:** FreeRTOS 的 `taskENTER_CRITICAL`/`taskEXIT_CRITICAL` 对应 LiteOS-M 的 **`LOS_IntLock()`**/`LOS_IntRestore()` 或 `SCHEDULER_LOCK`/`SCHEDULER_UNLOCK`。即在 LiteOS-M 中关中断来进入临界区，并解锁后再触发可能的调度。
* **空闲任务:** FreeRTOS 的 `vTaskIdleHook` 等对应 LiteOS-M 的 **空闲任务**（每核有一个 idle 任务由系统在最高优先级全 1 的队列中创建）。LiteOS-M 的 idle 任务在无其他任务可执行时循环运行（通常通过 `OsCurrTaskGet()->taskStatus == RUNNING` 保持 CPU 空闲）。 -->
...
本报告对FreeRTOS任务模块的Promela模型进行形式化数学描述。该模型实现了实时操作系统的核心任务调度、优先级管理、同步原语等功能。
...
$$TCB_t = \{ListItems[2], uxPriority, uxCriticalNesting\}$$

其中：
- $ListItems[2]$：双链表节点数组，$ListItems[0] = xState$，$ListItems[1] = xEvent$
- $uxPriority \in [0, configMAX\_PRIORITIES-1]$：任务优先级
- $uxCriticalNesting \in \mathbb{N}$：临界区嵌套计数

全局TCB数组：
$$TCBs: [0, promela\_TASK\_NUMBER] \rightarrow TCB_t$$

### 互斥量相关结构

当启用互斥量时（$configUSE\_MUTEXES = 1$），定义额外结构：

$$TCB\_MUTEXES_t = \{uxBasePriority, uxMutexesHeld\}$$

其中：
- $uxBasePriority$：基础优先级
- $uxMutexesHeld$：持有的互斥量数量

### 系统状态变量

$$\begin{align}
runTask &\in [NULL\_byte, promela\_TASK\_NUMBER] \\
uxTopReadyPriority &\in [0, configMAX\_PRIORITIES-1] \\
xTickCount &\in [0, 254] \\
xNextTaskUnblockTicks &\in [0, portMAX\_DELAY] \\
uxSchedulerSuspended &\in \mathbb{N} \\
xYieldPending &\in \{true, false\} \\
xPendedTicks &\in \{0, 1\}
\end{align}$$
...
$$pxReadyTasksLists: [0, configMAX\_PRIORITIES-1] \rightarrow List_t$$

其中每个优先级对应一个就绪任务链表。

### 延迟任务链表

$$pxDelayedTaskList: List_t$$

按照唤醒时间排序的延迟任务链表。

### 挂起任务链表

$$xSuspendedTaskList: List_t$$
...
对于非优化任务选择（$configUSE\_PORT\_OPTIMISED\_TASK\_SELECTION = 0$）：

$$taskRECORD\_READY\_PRIORITY(Priority) = \begin{cases}
uxTopReadyPriority \leftarrow Priority & \text{if } Priority > uxTopReadyPriority \\
\text{no change} & \text{otherwise}
\end{cases}$$

### 最高优先级任务选择

$$taskSELECT\_HIGHEST\_PRIORITY\_TASK() = \begin{cases}
local\_idx \leftarrow uxTopReadyPriority \\
\text{while } listLIST\_IS\_EMPTY(pxReadyTasksLists[local\_idx]) \text{ do} \\
\quad local\_idx \leftarrow local\_idx - 1 \\
runTask \leftarrow listGET\_OWNER\_OF\_NEXT\_ENTRY(pxReadyTasksLists[local\_idx]) \\
uxTopReadyPriority \leftarrow local\_idx
\end{cases}$$

### 任务添加到就绪链表

$$prvAddTaskToReadyList(pxTCB) = \begin{cases}
taskRECORD\_READY\_PRIORITY(TCB(pxTCB).uxPriority) \\
vListInsertEnd(pxReadyTasksLists[TCB(pxTCB).uxPriority], pxTCB)
\end{cases}$$
...
$$xTaskIncrementTick() = \begin{cases}
xTickCount \leftarrow xTickCount + 1 & \text{if scheduler not suspended} \\
xPendedTicks \leftarrow 1 & \text{if scheduler suspended}
\end{cases}$$

### 延迟任务处理

当 $xTickCount \geq xNextTaskUnblockTicks$ 时：

$$\forall task \in pxDelayedTaskList: \begin{cases}
\text{if } xTickCount \geq listGET\_LIST\_ITEM\_VALUE(task.ListItems[xState]) \\
\quad \text{remove task from delayed list} \\
\quad \text{add task to ready list} \\
\quad \text{update } xSwitchRequired \text{ if necessary}
\end{cases}$$
...
$$xTaskPriorityInherit(pxMutexHolder) = \begin{cases}
\text{if } TCB(pxMutexHolder).uxPriority < TCB(runTask).uxPriority \\
\quad TCB(pxMutexHolder).uxPriority \leftarrow TCB(runTask).uxPriority \\
\quad \text{update ready lists} \\
...
$$xTaskPriorityDisinherit(pxMutexHolder) = \begin{cases}
TCB\_uxMutexesHeld(pxMutexHolder) \leftarrow TCB\_uxMutexesHeld(pxMutexHolder) - 1 \\
\text{if } TCB\_uxMutexesHeld(pxMutexHolder) = 0 \text{ and} \\
\quad TCB(pxMutexHolder).uxPriority \neq TCB\_uxBasePriority(pxMutexHolder) \\
\quad TCB(pxMutexHolder).uxPriority \leftarrow TCB\_uxBasePriority(pxMutexHolder) \\
\quad \text{update ready lists}
\end{cases}$$
...
$$S = \{Ready, Running, Blocked, Suspended, Exited\}$$
...
$$\begin{align}
\delta(Ready, Schedule) &= Running \\
\delta(Running, Yield) &= Ready \\
\delta(Running, Block) &= Blocked \\
\delta(Running, Suspend) &= Suspended \\
\delta(Blocked, Unblock) &= Ready \\
\delta(Suspended, Resume) &= Ready \\
\delta(x, Delete) &= Exited \;(x \neq Exited)
\end{align}$$
...
1. **当前任务不变量**：
   $$runTask \neq NULL\_byte \Rightarrow TCB(runTask) \in ReadyTasks$$

2. **优先级不变量**：
   $$\forall t \in ReadyTasks: TCB(t).uxPriority \leq TCB(runTask).uxPriority$$

3. **就绪链表一致性**：
   $$\forall p \in [0, configMAX\_PRIORITIES-1]: \\
   \forall t \in pxReadyTasksLists[p]: TCB(t).uxPriority = p$$

### 时间不变量

1. **延迟任务排序**：
   $$\forall t_1, t_2 \in pxDelayedTaskList: \\
   index(t_1) < index(t_2) \Rightarrow wakeup\_time(t_1) \leq wakeup\_time(t_2)$$

2. **时钟单调性**：
   $$xTickCount' \geq xTickCount \text{ (在正常操作中)}$$ -->
