...
/* 轮转切换见证：P1 -> P2 -> P1（展示详细步骤） */
ltl p1_p2_round_robin_cycle {
  NEVER(
    EVENTUALLY_THEN(
        IN_USER_STATE(2) && SYSTICK_INT,          /* P1执行，SysTick到来 */
        THEN(IN_SYSTICK(2),                       /* 进入SysTick（栈=P1） */
        THEN(IN_PENDSV(2),                        /* 进入PendSV（栈=P1） */
        THEN(IN_PENDSV(3),                        /* switch_context 切到P2（栈=P2） */
        THEN(IN_USER_STATE(3),                    /* 返回P2用户态 */
        THEN(IN_SYSTICK(3),                       /* 再次SysTick到来（P2被打断） */
        THEN(IN_PENDSV(3),                        /* 进入PendSV（栈=P2） */
             IN_USER_STATE(2)                     /* 切回并返回P1用户态 */
        ))))))
    )
  )
}
